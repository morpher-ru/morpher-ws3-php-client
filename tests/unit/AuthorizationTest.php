...
class AuthorizationTest extends TestCase
{
	...
	public function testServerError500(string $method, string $requestResult, callable $callback): void
	{
		...
		$testMorpher = MorpherTestHelper::createMockMorpherWithException(new \GuzzleHttp\Exception\ServerException('Error 500',
			new \GuzzleHttp\Psr7\Request($method, 'test'), new \GuzzleHttp\Psr7\Response(500, [], '')));
	...
	public function testParse_NetworkError1(string $method, string $requestResult, callable $callback): void
	{
		...
		$testMorpher = MorpherTestHelper::createMockMorpherWithException(new \GuzzleHttp\Exception\ConnectException('connection cannot be established',
	...
	public function testParse_NetworkError2(string $method, string $requestResult, callable $callback): void
	{
		...
		$testMorpher = MorpherTestHelper::createMockMorpherWithException(new \GuzzleHttp\Exception\RequestException('connection cannot be established',
			new \GuzzleHttp\Psr7\Request($method, 'test')));
		$callback($testMorpher);
	}
	...
	public function assertAuthorizationNoToken(\Psr\Http\Message\RequestInterface $request,
